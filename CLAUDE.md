...
A local Streamlit app that pulls raid reports from the Warcraft Logs v2 GraphQL API and produces CSV summaries of per-player **deaths** and **hits taken** (damage-event count, plus the underlying damage total; Mythic difficulty, wipes only), grouped by boss/ability and date. Two analysis sources: a single guild (by URL) or the top-N world-progress guilds for a boss (from a stored ranking).
...
- **`src/*_fetcher.py`** — thin query wrappers over `run_wcl_query`: `calendar_fetcher` (guild report list), `deaths_fetcher` / `damage_taken_fetcher` (per-player event aggregation for a report + boss + ability), `survivability_fetcher`, `guild_rankings_fetcher` (`get_encounter_zone_id`, live ranking fetch), and `player_details_fetcher` (`get_player_class_specs` — per-report class/spec via WCL's `playerDetails` field, with an `icon`-parsing fallback if `type`/`specs` come back empty). `deaths_fetcher.fetch_fights_and_actors()` combines the fights and actors queries (independent reads off the same report node) into one round trip.

- **`src/report_cache.py`** — the per-run cache sitting above the fetchers, shared by all three analysis paths (single-guild, live multi-guild, overnight) so the same report/guild is never queried twice for data that doesn't depend on which ability/metric is being analysed: guild report lists (keyed by guild+date-range+zone — selecting both Deaths and Hits used to re-fetch this per metric pass), fights+actors (`get_report_fights`/`get_report_actors` share one combined round trip via `deaths_fetcher.fetch_fights_and_actors` — whichever runs first for a report warms both caches), class/specs, and death/damage events (ability-filtered server-side, so distinct abilities get distinct cache entries). `reset_report_caches()` clears everything at the start of a run. A combined deaths+damage events fetch (one aliased query warming both caches when a run wants both metrics) was tried and **reverted** after it caused a real, unreproduced-in-testing stall under live multi-report concurrency — see `docs/api-efficiency-findings.md` for what was tried, why it was reverted, and what to check before retrying it.
...
- **`ui/app.py`** — the whole page and all orchestration. Flow: env gate → rate-limit widget → `render_input_settings()` → metric radio (Deaths / Hits / Both) → branch on `source_settings["mode"]`:
...
- **`src/estimate.py`** — pure, API-free heuristics for "how many points / how long will this range take": `estimate_points`, `estimate_compute_seconds`, `plan()` (adds budget feasibility + wait time for hourly resets), `format_duration`. **Single source of truth** for both the live estimate in the ranking tab and the pre-flight in `compute_multi_guild_cache`. `POINTS_PER_GUILD = 160` / `SECONDS_PER_GUILD = 75.0` were calibrated against a real live run (a first-kill-cutoff pass, 19 progression report-days) — still a rough, guild-shape-dependent stand-in for something that scales with report-days-in-range, not a guarantee. `overnight.py`'s `_MAX_CHUNK` and `live_runner.py`'s `MAX_CHUNK` are both derived from this constant, so recalibrating it also reshapes those caps — see the comments there.

- **`src/overnight.py` + `overnight_run.py`** — the auto-resume runner for ranges too big for one hourly window. `_next_chunk_size()` fills each batch to whatever the *currently remaining* budget affords (not a fixed chunk size — draining a hopeful window down to under one guild's worth of points before ever waiting, capped by `_MAX_CHUNK`/job.json's `chunk_size`, itself derived from `estimate.POINTS_PER_GUILD` so it can't silently exceed a full hourly window again). Once even a single guild can't be afforded, it sleeps straight through to WCL's own reset countdown (`points_reset_in`) rather than polling; `poll_seconds` only paces retries when the budget *check itself* fails outright. Writes a job directory under `output/overnight/<stamp>_r<a>-<b>/` containing `job.json`, live `status.json` (polled by the UI), `checkpoint.json` (processed ranks + rows, so a crash/reboot resumes), `run.log`, per-(boss, metric) CSVs, and an optional `STOP` sentinel for a clean early exit. No Streamlit import, so it has no UI dependency. The ranking tab's 🌙 expander launches it in its **own visible console window** (`CREATE_NEW_CONSOLE`, detached via `CREATE_NEW_PROCESS_GROUP` so it survives closing the browser tab/Streamlit app — but closing that console window kills the run, an intentionally abrupt stop rather than a clean checkpointed one) and renders its status file; `overnight_run.py` tees its own stdout/stderr to both that console and `run.log` (`_tee_output_to_log`) so the two always agree regardless of how it was launched.
